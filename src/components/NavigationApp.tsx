...
export default function NavigationApp() {
  ...
  const mutate = async (body: object) => {
    try {
      ...
      setMenuEditor(null)
      setSiteEditor(null)
    } catch (error) { setNotice(error instanceof Error ? error.message : '无法保存。') }
  }
  ...
  const toggleFavorite = async (site: Site) => {
    ...
    try {
      const result = await api('/api/navigation', { method: 'POST', body: JSON.stringify({ action: favorited ? 'remove-favorite' : 'add-favorite', url: site.url }) }) as Payload
      setData(result)
    } catch (error) { setNotice(error instanceof Error ? error.message : '无法更新收藏。') }
  }
      ...
      <div className="relative mx-auto flex min-h-screen max-w-[1680px] flex-col px-3 py-3 md:px-5 md:py-5">
        ...
        <div className="flex flex-1 flex-col overflow-hidden rounded-xl border border-white/[.08] bg-[#0d1016] shadow-2xl shadow-black/25 lg:flex-row">
          <aside className={`w-full shrink-0 border-b border-white/[.08] bg-[#12161e] p-3 transition-[width,padding] duration-300 lg:border-b-0 lg:border-r lg:bg-[#10141b] ${sidebarCollapsed ? 'hidden lg:block lg:w-0 lg:overflow-hidden lg:border-r-0 lg:p-0' : 'lg:w-60'}`}>
             <div className="mb-4 flex items-center justify-between px-2"><span className="text-xs font-semibold uppercase tracking-[.18em] text-slate-500">收藏集</span>{data.authenticated && editMode && <button title="添加收藏集" onClick={() => setMenuEditor({ name: '', parentId: null })} className="icon-button">➕</button>}</div>
             <nav className="space-y-1">{roots.map((root) => { const children = menus.filter((menu) => menu.parentId === root.id); const open = expanded.includes(root.id); return <div key={root.id}><div className={`group flex items-center gap-1 rounded-lg ${activeMenu === root.id ? 'bg-blue-500/15 text-blue-100' : 'text-slate-300 hover:bg-white/5'}`}><button className="p-2 text-slate-500" onClick={() => setExpanded((items) => items.includes(root.id) ? items.filter((id) => id !== root.id) : [...items, root.id])}>{children.length ? open ? <ChevronDown size={15} /> : <ChevronRight size={15} /> : <span className="block w-[15px]" />}</button><button className="flex-1 px-1 py-2 text-left text-sm font-medium" onClick={() => setActiveMenu(root.id)}>{root.name}</button>{data.authenticated && editMode && <div className="hidden gap-1 pr-1 group-hover:flex"><button title="添加子菜单" className="icon-button" onClick={() => setMenuEditor({ name: '', parentId: root.id })}>➕</button><button title="编辑菜单" className="icon-button" onClick={() => setMenuEditor({ id: root.id, name: root.name })}>✏️</button><button title="删除菜单" className="icon-button text-rose-300" onClick={() => deleteItem('menu', root.id)}>🗑️</button></div>}</div>{open && children.map((child) => <div key={child.id} className={`group ml-6 flex items-center rounded-lg ${activeMenu === child.id ? 'bg-blue-500/10 text-blue-100' : 'text-slate-400 hover:bg-white/5'}`}><button onClick={() => setActiveMenu(child.id)} className="flex-1 px-3 py-2 text-left text-sm">{child.name}</button>{data.authenticated && editMode && <div className="hidden gap-1 pr-1 group-hover:flex"><button title="编辑菜单" className="icon-button" onClick={() => setMenuEditor({ id: child.id, name: child.name })}>✏️</button><button title="删除菜单" className="icon-button text-rose-300" onClick={() => deleteItem('menu', child.id)}>🗑️</button></div>}</div>)}</div>})}</nav>
          </aside>
           <section className="min-w-0 flex-1 p-5 md:p-8"><div className="mb-8 flex items-end justify-between gap-4"><h1 className="text-3xl font-semibold tracking-tight md:text-4xl">{activeMenu ? menus.find((menu) => menu.id === activeMenu)?.name : '全部收藏'}</h1>{data.authenticated && editMode && <button title="添加网站" className="button-primary" onClick={() => setSiteEditor({ menuId: activeMenu || menus[0]?.id || '', name: '', description: '', url: '' })}>➕</button>}</div>
            {loading ? <div className="grid place-items-center py-28 text-slate-400">正在加载你的导航…</div> : visibleSites.length ? <><div className="columns-1 gap-4 sm:columns-2 xl:columns-3">{renderedSites.map((site) => <article key={site.id} className="group relative mb-4 break-inside-avoid rounded-2xl border border-white/10 bg-gradient-to-b from-white/[.07] to-white/[.025] p-5 transition hover:-translate-y-0.5 hover:border-blue-400/30 hover:shadow-xl hover:shadow-blue-950/20"><a href={site.url} target="_blank" rel="noreferrer" className="block"><div className="mb-5 flex items-start justify-between"><div className="grid h-10 w-10 place-items-center overflow-hidden rounded-xl border border-white/10 bg-gradient-to-br from-slate-200/80 via-slate-400/40 to-slate-700/70 animate-pulse"><img src={`/api/icon?url=${encodeURIComponent(site.iconUrl)}`} alt="" loading="lazy" className="h-full w-full bg-white p-1.5" onLoad={(event) => event.currentTarget.parentElement?.classList.remove('animate-pulse')} onError={(event) => { event.currentTarget.style.display = 'none' }} /></div><span className="max-w-[55%] truncate text-xs text-slate-500">{new URL(site.url).host}</span></div><h2 className="font-semibold text-slate-100">{site.name}</h2><p className="mt-2 line-clamp-2 min-h-10 text-sm leading-5 text-slate-400">{site.description}</p></a>{data.authenticated && editMode && <div className="mt-4 flex gap-2 border-t border-white/10 pt-3"><button title="编辑网站" className="icon-button text-xs" onClick={() => setSiteEditor(site)}>✏️</button><button title="删除网站" className="icon-button text-xs text-rose-300" onClick={() => deleteItem('site', site.id)}>🗑️</button></div>}</article>)}</div>{hasMoreSites && <div ref={loadMoreRef} className="grid h-20 place-items-center text-sm text-slate-500"><span className="rounded-full border border-white/10 bg-white/[.03] px-3 py-1.5">正在加载更多网站…</span></div>}</> : <div className="grid min-h-72 place-items-center rounded-2xl border border-dashed border-white/15 bg-white/[.02] text-center"><div><Globe2 className="mx-auto mb-3 text-slate-600" size={32} /><p className="font-medium text-slate-300">这里还没有网站</p><p className="mt-1 text-sm text-slate-500">{data.authenticated ? '添加一个网站开始建设这个收藏集。' : '登录以创建你自己的导航。'}</p></div></div>}
          </section>
...
function AuthDialog({ onClose, onDone }: { onClose: () => void; onDone: () => void }) { const [error, setError] = useState(''); const [loading, setLoading] = useState(false); const submit = async (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); if (loading) return; setLoading(true); setError(''); const form = new FormData(event.currentTarget); try { await api('/api/auth/login', { method: 'POST', body: JSON.stringify({ username: form.get('username'), password: form.get('password') }) }); onDone() } catch (error) { setError(error instanceof Error ? error.message : '无法继续。') } finally { setLoading(false) } }; return <Dialog title="欢迎回来" onClose={onClose}><form className="space-y-4" onSubmit={submit}><Field label="用户名" name="username" placeholder="例如：zhang_san" required /><Field label="密码" type="password" name="password" placeholder="至少 8 个字符" minLength={8} required />{error && <p className="text-sm text-rose-300">{error}</p>}<button className="button-primary w-full justify-center" type="submit" disabled={loading}>{loading ? '处理中...' : '登录'}</button></form></Dialog> }
function MenuDialog({ value, onClose, onSave }: { value: { name: string }; onClose: () => void; onSave: (name: string) => void }) { const [name, setName] = useState(value.name); return <Dialog title={value.name ? '重命名菜单' : '新建菜单'} onClose={onClose}><form className="space-y-4" onSubmit={(event) => { event.preventDefault(); onSave(name) }}><Field label="菜单名称" value={name} onChange={(event) => setName(event.target.value)} required autoFocus /><button className="button-primary w-full justify-center">保存菜单</button></form></Dialog> }
function SiteDialog({ value, menus, onClose, onSave }: { value: Partial<Site>; menus: Menu[]; onClose: () => void; onSave: (site: Partial<Site>) => void }) { const [site, setSite] = useState(value); const update = (key: keyof Site, value: string) => setSite((current) => ({ ...current, [key]: value })); return <Dialog title={site.id ? '编辑网站' : '添加网站'} onClose={onClose}><form className="space-y-4" onSubmit={(event) => { event.preventDefault(); onSave(site) }}><Field label="名称" value={site.name || ''} onChange={(event) => update('name', event.target.value)} required /><Field label="描述" value={site.description || ''} onChange={(event) => update('description', event.target.value)} required /><Field label="网站链接" type="url" value={site.url || ''} onChange={(event) => update('url', event.target.value)} placeholder="https://example.com" required /><label className="block text-sm text-slate-300">所属菜单<select value={site.menuId || ''} onChange={(event) => update('menuId', event.target.value)} className="mt-1.5 w-full rounded-xl border border-white/10 bg-[#171923] px-3 py-2.5 text-sm outline-none focus:border-blue-400">{menus.map((menu) => <option key={menu.id} value={menu.id}>{menu.parentId ? '↳ ' : ''}{menu.name}</option>)}</select></label><button className="button-primary w-full justify-center">保存网站</button></form></Dialog> }
